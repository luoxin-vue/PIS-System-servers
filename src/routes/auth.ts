...
authRouter.post(
  ...
  asyncRoute(async (req: Request, res: Response) => {
    ...
    if (!username || !password) {
      sendError(res, 400, 'AUTH_CREDENTIALS_REQUIRED', 'Username and password required');
      return;
    }
    const db = getClient();
    ...
    if (!user || !bcrypt.compareSync(password, user.password_hash)) {
      sendError(res, 401, 'AUTH_INVALID', 'Invalid username or password');
      return;
    }
...
authRouter.post(
  ...
  asyncRoute(async (req: Request, res: Response) => {
    ...
    if (password.length < 6) {
      sendError(res, 400, 'AUTH_PASSWORD_TOO_SHORT', 'Password must be at least 6 characters');
      return;
    }
    const db = getClient();
    const existing = row0(await db.execute('SELECT id FROM users WHERE username = ?', [username]));
    if (existing) {
      sendError(res, 409, 'AUTH_USERNAME_TAKEN', 'Username already exists');
      return;
    }
